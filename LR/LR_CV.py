...
# Output report
REPORT_FILE = "LR_baseline_report.txt"
...
# Common LR settings
COMMON_LR = dict(max_iter=10000, random_state=RNG_SEED)


# Base pipeline: scaler + LR
BASE_LR_PIPE = Pipeline([
    ("imputer", SimpleImputer(strategy="median")),
    ("scaler", StandardScaler()),                # fit only on TRAIN data
    ("clf", LogisticRegression(**COMMON_LR)),
])



# Parameter space for grid
LR_PARAM_SPACE = {
    "clf__C": [0.0001, 0.0005, 0.001, 0.01, 0.05, 0.1],
    "clf__penalty": ["l2"],
    # both fine for binary, l2-penalized
...
LR_PARAM_GRID = make_param_grid(LR_PARAM_SPACE, max_combos=40, seed=RNG_SEED)
...
def tune_and_calibrate(X_tr: np.ndarray, y_tr: np.ndarray, g_tr: np.ndarray, 
                       param_grid: List[Dict], inner_splits: int = INNER_SPLITS, 
seed: int = RNG_SEED) -> Tuple[Dict, IsotonicRegression, float, float, Dict[float, float], Dict[float, float], np.ndarray, np.ndarray]:
    """
    ...
    for params in param_grid:
        ...
        for tr_idx, va_idx in sgkf.split(X_p, y_p, groups=g_p):
            ...
            # ROC AUC on inner validation fold
            # (guard against folds with a single class)
            #y_va = y_p[va_idx]
            #if len(np.unique(y_va)) < 2:
            #    continue
            #scores.append(roc_auc_score(y_va, p_val))
    ...
    final_model = clone(BASE_LR_PIPE)
...
def main():
    ...
    for k, (tr_idx, ts_idx) in enumerate(outer.split(X, y, groups=groups), start=1):
        ...
        best_params, calibrator, tau_wave, tau_spk, qhat_wave, qhat_spk, mask_proper, mask_cp = tune_and_calibrate(
            Xtr, ytr, gtr, LR_PARAM_GRID, INNER_SPLITS, RNG_SEED + k)   
        print(f"  proper-train N={mask_proper.sum()} | CP-calib N={mask_cp.sum()}")
        print(f"  best LR params: {best_params} | tau_wave={tau_wave:.3f} | tau_spk={tau_spk:.3f}")

        # Build best pipeline for this outer fold
        best_lr = clone(BASE_LR_PIPE)
        ...
        # later: before calibration or directly for test
        ...
        # also keep arrays for a pooled reliability plot (outer test is non-overlapping across folds)
        ...
        # ---- Cougher-level (mean prob per speaker) ----
    ...
    # Speaker-level reliability (pooled outer test)
    ...
    df_cal.to_csv('LR_calibration.csv', sep='\t')
    
    summary = df_cal.drop(columns=["outer_fold"]).agg(["mean", "std"]).T
    print("\n")
    print(summary)  # or export to csv / latex
    
    macro, pooled = summarize_over_folds(df_cond)
    macro.to_csv('LR_macro.csv', sep='\t')
    pooled.to_csv('LR_pooled.csv', sep='\t')


    # Write everything to a text report
    with open(REPORT_FILE, "w", encoding="utf-8") as f:
        f.write("Nested CV Logistic Regression baseline\n")
